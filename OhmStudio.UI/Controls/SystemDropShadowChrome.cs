...
namespace OhmStudio.UI.Controls
{
    public sealed class SystemDropShadowChrome : Decorator
    {
        public static readonly DependencyProperty ColorProperty =
            DependencyProperty.Register("Color", typeof(Color), typeof(SystemDropShadowChrome), new FrameworkPropertyMetadata(Color.FromArgb(113, 0, 0, 0), FrameworkPropertyMetadataOptions.AffectsRender, ClearBrushes));

        public static readonly DependencyProperty CornerRadiusProperty =
            DependencyProperty.Register("CornerRadius", typeof(CornerRadius), typeof(SystemDropShadowChrome), new FrameworkPropertyMetadata(default(CornerRadius), FrameworkPropertyMetadataOptions.AffectsRender, ClearBrushes), IsCornerRadiusValid);
        ...
        private static void ClearBrushes(DependencyObject sender, DependencyPropertyChangedEventArgs e)
        {
            ((SystemDropShadowChrome)sender)._brushes = null;
        }
        ...
        private Brush[] GetBrushes(Color color, CornerRadius cornerRadius)
        {
            if (_commonBrushes == null)
            {
                lock (_resourceAccess)
                {
                    if (_commonBrushes == null)
                    {
                        _commonBrushes = CreateBrushes(color, cornerRadius);
                        _commonCornerRadius = cornerRadius;
                    }
                }
            }

            if (color == ((SolidColorBrush)_commonBrushes[4]).Color && cornerRadius == _commonCornerRadius)
            {
                _brushes = null;
                return _commonBrushes;
            }

            _brushes ??= CreateBrushes(color, cornerRadius);
